...
stationStatusesEditor.on('open', function() {
    openStationStatusVals = stationStatusesEditor.get();
    currentStationStatusId = openStationStatusVals.id;
});


// Pre-submit report handler: prune data and remove empty fields
stationStatusesEditor.on('preSubmit', function(e, data, action) {

    
    if (action === 'create') {
        // Remove empty fields from data for new report
        Object.entries(data.data).forEach(([index, report]) => {
            // Remove empty data sets
            pruneEmptyFields(report);
        });
    } else if (action === 'edit') {
        // Remove fields that have not changed for edit report
        Object.entries(data.data).forEach(([index, report]) => {
            Object.entries(report).forEach(([key, value]) => {
                if (value === openStationStatusVals[key]) {
